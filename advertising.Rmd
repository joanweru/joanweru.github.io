...
*  Here, the **Residual standard error** are \$1,660.

* The **adjusted r-squared** value measures how much of the variation in the response variable is explained  by the set of predictors. In this case 90.7% of the variation of in `Sales` is explained by the set of predictors. But this does not necessarily mean that the model will predict new data accurately.
...
Our model predicted our test data with an accuracy of 94.95% .It seems our model is good for predicting but it wouldn't hurt to improve our prediction accuracy.
...
* Also, we see that 50% of our **Residuals standard error** is \$1,566 This is an improvement from our previous model.
...
In our improved model, the model accuracy is 95.52%, not quite a big improvement from our previous model, but an improvement nonetheless.
