...
From the plots above it seems only `Newspaper` has outliers. Lets view a scatter plot if the outlier has any effect on it's relationship with `Sales`.
...
There seems to be a significant correlation between `Sales` and `TV` as well as `Sales` and `Radio`, but not so much with `Sales` and `Newspaper`. We could decide to drop it we'll  check to see if there are any **interaction effects** between `Newspaper` and any other predictor variable.
...
* The **residuals** sections provides summary statistics for the errors in our prediction. Here, we see that 50% of our errors fall between   \$ 1,303  over the true value and \$ 1,172 under the true value.

* The **adjusted r-squared** value measures how much of the variation in the response variable is explained  by the set of predictors. In this case 89.3% of the variation of in `Sales` is explained by the set of predictors. But this does not necessarily mean that the model will predict new data accurately.

Let's tale a look at the **model accuracy**.
...
Our model predicted our test data with an accuracy of 94% .It seems our model is good for predicting but it wouldn't hurt to improve our prediction accuracy.
...
* Also, we see that 50% of our **residuals** fall between \$ 1,0301 over the true value and \$ 3,526 under the true value. This is an improvement from our previous model.

* The **adjusted r-squared**  is 0.9192  Therefore in this case 92% of the variation in `Sales` is explained by the set of predictors. This is also an improvement from our previous model.
...
In our improved model, the model accuracy is 95%, not quite a big improvement from our previous model, but an improvement nonetheless.
