...
* `Attrition ﬂag` -  If the credit card subscription has been cancelled or not("*Attrited Customer*"=it is cancelled, "*Existing Cutomer*"=it is not cancelled)
...
* ` Total Transaction Count` - Change in transaction comparing the 4th quarter against 1st
...
```{r,out.width="55%",fig.keep="hold"}

bank%>%ggplot(aes(x=Income_Category,fill=Attrition_Flag))+geom_bar(position = "fill")+labs(title="Income category",y="Percent",x="Income category")+scale_fill_brewer(palette = "Set2")+scale_y_continuous(breaks=seq(0,1,.2),label=percent)+theme(axis.title.y = element_blank(),legend.title = element_blank(),plot.margin = unit(c(1,1,0,1),"cm"))+theme_fivethirtyeight()

bank%>%ggplot(aes(x=Card_Category,fill=Attrition_Flag))+geom_bar(position = "fill")+theme_fivethirtyeight()+labs(title="Card category",y="Percent",x="Card category")+scale_fill_brewer(palette = "Set2")+scale_y_continuous(breaks=seq(0,1,.2),label=percent)

bank%>%ggplot(aes(x=Education_Level,fill=Attrition_Flag))+geom_bar(position = "fill")+theme_fivethirtyeight()+labs(title="Education Level",y="Percent",x="Education level")+scale_fill_brewer(palette = "Set2")+scale_y_continuous(breaks=seq(0,1,.2),label=percent)

bank%>%ggplot(aes(x=Gender,fill=Attrition_Flag))+geom_bar(position = "fill")+theme_fivethirtyeight()+labs(title="Gender",y="Percent",x="Gender")+scale_fill_brewer(palette = "Set2")+scale_y_continuous(breaks=seq(0,1,.2),label=percent)

bank%>%ggplot(aes(x=Marital_Status,fill=Attrition_Flag))+geom_bar(position = "fill")+theme_fivethirtyeight()+labs(title="Marital Status",y="Percent",x="Marital")+scale_fill_brewer(palette = "Set2")+scale_y_continuous(breaks=seq(0,1,.2),label=percent)
```
...
```{r,out.width="50%",fig.keep="hold"}
bank%>%filter(Attrition_Flag=="Attrited Customer")%>%count(Education_Level)%>%rename("Attrited Customer"=n)%>%treemap(bank2,index = c("Education_Level"),vSize = "Attrited Customer",palette = "Set2")

bank2=bank%>%filter(Attrition_Flag=="Attrited Customer")%>%count(Card_Category)%>%rename("Attrited Customer"=n)%>%treemap(bank2,index = c("Card_Category"),vSize = "Attrited Customer",palette = "Set2")

bank2=bank%>%filter(Attrition_Flag=="Attrited Customer")%>%count(Gender)%>%rename("Attrited Customer"=n)%>%treemap(bank2,index = c("Gender"),vSize = "Attrited Customer",palette = "Set2")

bank2=bank%>%filter(Attrition_Flag=="Attrited Customer")%>%count(Marital_Status)%>%rename("Attrited Customer"=n)%>%treemap(bank2,index = c("Marital_Status"),vSize = "Attrited Customer",palette = "Set2")
```
